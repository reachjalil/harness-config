...
- Preserve the one-way projection model. Source of truth is `./.harness`;
  target folders are generated outputs, not source repositories.
- Use the standard filenames exactly: `.harnessIgnore`, `.harnessProfile`,
  `.harnessProfileRoot`, and `.harnessComposable`.
- Treat `.harnessProfileRoot` as profile source only. It must live under
  `.harness`, must not be projected as a resource item, and must overlay
  resources or `[dir]` outputs by logical source path.
...
- `[mutable]` files are created once, then left untouched until
  `--force-mutable`.
- `[dir]` composition writes root files, follows `.ref`, and merges outputs
...
- `.harnessProfileRoot` outside `.harness`,
- `[dir]` ref cycles, missing refs, mixed file/directory conflicts, and target
  root overlaps.
