...
- Resource kinds are directory names, not schema tables. Direct files under the
  configured resources source are valid.
...
- `.harnessRef` imports another composable leaf; `.ref` is not a standard
  filename.
- `[dir]` outputs repo-relative files from an explicit dir source. Outputs that
