...
- `harnessc explain <path>` explique comment un chemin source ou de sortie participe au plan de projection courant, y compris les chemins source gagnants, les racines source configurées, les sorties dir, les diagnostics bloquants, les décisions `.harnessIgnore` et les décisions de propriété `.harnessMutable`. La sortie JSON inclut des traces d'ignore source et en sortie cible pour qu'un appelant puisse distinguer une exclusion racine, une ré-inclusion source-locale plus profonde, une ré-inclusion logique profil-locale et une limite finale en sortie cible.
