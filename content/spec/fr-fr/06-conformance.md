...
Les outils conformes DEVRAIENT (SHOULD) rapporter des codes de diagnostic lisibles par machine à côté des messages humains. Le catalogue des codes v1 est maintenu dans [Diagnostics](https://github.com/JalilAlaichi/harness-config/blob/main/docs/DIAGNOSTICS.md). Les outils qui émettent un code `harness.*` DOIVENT (MUST) utiliser un code de ce catalogue. Les outils PEUVENT (MAY) émettre des codes dans leur propre espace de noms (par exemple `my-tool.*`) pour les conditions hors du standard.
