...
Lorsqu'un dépôt ignore par gitignore les sorties cibles générées, il SHOULD garder les instructions d'activation trackées, telles qu'une note d'instructions racine, une étape README ou un script, pour qu'un nouveau checkout puisse valider et régénérer ces sorties. Les racines source configurées partagées qui rendent les cibles reproductibles SHOULD rester trackées ; les racines locales privées ou expérimentales telles que `.harness/local/` MAY être gitignored lorsque le dépôt les traite intentionnellement comme des superpositions locales au développeur.
...
- Les dépôts qui gitignored les surfaces de harness vivantes SHOULD garder les instructions d'activation trackées et SHOULD NOT gitignored les racines source configurées partagées requises pour régénérer ces surfaces. Les racines source locales au développeur MAY être gitignored lorsqu'elles sont intentionnellement en dehors de la source de vérité partagée.
