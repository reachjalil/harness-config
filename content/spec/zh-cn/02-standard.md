...
- `./.harness` 约定根，
- 仅路径 target、有序 `[[resources]]` 源根、有序 `[[dir]]` 源根和顶级扩展声明的所选 TOML manifest schema，
- 配置过的 resources 源树，
- 由 target 派生的 override 文件夹，
- 拷贝投影（在固定输入下幂等），
- dir 组合（`.harnessComposable` 叶）和投影到相对仓库路径的文件的拷贝契约，
- `.harnessIgnore` 投影 ignore 文件，包括仓库根规则、源本地规则和目标输出本地规则，
...
每个 `[[resources]]` 表 MUST 仅包含 `path`。路径 MUST 是仓库本地，MUST 解析为仓库内部，MUST NOT 包含 `..` 段。Manifest MUST NOT 包含单个 `[resources]` 表或任何 `[resources.<kind>]` 表；资源类型保持为源树名称，不是 manifest schema 条目。
...
每个 target 都是显式的。Harness config 不保留、偏好或暗示任何 runtime target 文件夹名称。所选 manifest 中的每个 `[[targets]]` 条目声明一个仓库本地 target 路径并 MUST 仅包含 `path`。
...
可选的顶级 `[activation]` 表包含标准激活策略。当省略时，所有字段使用它们的默认值。
...
工具 SHOULD NOT 在 v1 的所选 manifest 中引入按 target 的资源映射。把 target 声明保持为仅路径并把源根有序地放在顶级，保留了投影过滤的单一位置，并使 dry-run 输出更易推理。
...
2. 所选版本化 manifest，
3. 仓库根 `.harnessIgnore`，
4. 仓库根 `.harnessMutable`，
5. 清理策略（保留未管理项 vs 删除它们），
6. mutable 策略（跳过 mutable 文件 vs 强制重新投影），
7. target 符号链接策略（冲突 vs 替换）。

**幂等性（可测试属性）。** 设 `T_n` 是声明的 target 在第 `n` 次激活对不变的输入集 (1)–(7) 之后的磁盘上树。对于每个 `n ≥ 2`：

- `T_n` 中的文件集合 MUST 等于 `T_1` 中的集合，
- `T_n` 中的每个受管理（非 mutable）文件 MUST 与 `T_1` 中的对应文件字节相同，
- `T_1` 中存在的每个 mutable 文件 MUST 在 `T_n` 中仍然存在，并具有它在激活 `n − 1` 结束时所具有的相同字节（即 runtime 拥有它；激活不写入它），并且
...
- 激活对固定源树、所选 manifest、profile 选择器、ignore 规则、mutable 规则、清理策略和 mutable 策略是确定的。
...
投影 MUST 按以下顺序处理 resources 树：

1. 拷贝规范资源文件，排除 target 根 override 文件夹和项级 override 文件夹。
2. 如果存在，合并匹配的 target 根 override 文件夹。
3. 如果存在，合并匹配的项级 target override 文件夹。
4. 从输出路径中剥离 override 文件夹段。
5. 在每个源文件进入投影之前对它应用 `.harnessIgnore` 规则。
...
每个 `[[dir]]` 表 MUST 仅包含 `path`。Manifest MUST NOT 包含单个 `[dir]` 表。如果未声明 `[[dir]]` 条目，则不进行 dir 组合或拷贝。缺失的 dir 源是有效的空层。
...
模式使用与 `.harnessIgnore` 相同的语法、本地性、否定、锚点、仅目录后缀和最后匹配获胜优先级。根文件是相对仓库的，可以匹配源路径或目标输出路径。源本地和 profile 本地文件相对于它们的逻辑源目录解释。目标输出 `.harnessMutable` 文件不属于 v1；mutable 声明属于源，不属于活动 target。
...
- `[ignore]` 在 `.harnessMutable` 中不被支持；ignore 规则属于 `.harnessIgnore`。
- 特定 target 的标题不被支持，原因与它们在 `.harnessIgnore` 中不被支持的原因相同。
...
Profile 覆盖是由 `.harnessProfile` 文件选择的可选源覆盖。`.harnessProfile` 文件是 UTF-8 文本。在修剪每行的空白并忽略空白行后，它 SHOULD 包含零或一个 profile 名称。零个 profile 名称为该输出子树选择没有 profile。多于一个非空行 SHOULD 产生警告，工具 MAY 为了兼容使用第一个 profile 名称。仓库根 `.harnessProfile` 全局应用；目标/输出本地 `.harnessProfile` 应用于它的目录和后代，对于任何输出路径，最近的选择器获胜。
...
在投影期间，先跨 manifest 顺序中的 resources 源考虑通用基础源文件，然后通用活动 profile 文件，然后跨 manifest 顺序中的 resources 源的由 target 派生的 override 文件，然后匹配 target override 内的活动 profile 文件。因此通用 profile 覆盖不能替换特定 target 的 override（如 `.codex`）；特定 profile 的 `.codex` override 可以。如果多个活动 profile 根投影同一逻辑文件，工具 SHOULD 警告并 MAY 使用确定的最后获胜顺序。Profile 本地 `.harnessIgnore` 和 `.harnessMutable` 文件匹配逻辑覆盖路径，不是存储路径。例如，在 `.harness/profiles/personal/dir/AGENTS.md/.harnessIgnore` 的 ignore 文件就像它位于 `.harness/dir/AGENTS.md/.harnessIgnore` 一样应用，因此它可以在添加 profile 部分之前抑制基础可组合部分。

作为 profile 根的物理祖先的源本地 `.harnessIgnore` 文件也在 profile 根映射到其逻辑覆盖路径之前应用。例如，`.harness/kits/.harnessIgnore` 可以排除活动 `deploy` profile 中的 `.harness/kits/deploy/**/.harnex/` 元数据，即使该 profile 根下的文件覆盖逻辑路径（如 `.harness/resources` 或 `.harness/dir`）。
...
- 工具提供的只读路径自省，MUST 从与激活相同的所选 manifest、配置过的源根、profile 选择器、ignore 规则、mutable 规则、mutable 策略和投影模型派生。
...
- 激活 MUST 对相同的所选 manifest、配置过的源根、`.harnessIgnore`、`.harnessMutable`、参与资源、清理策略和 mutable 策略幂等。
