...
- 选中的 manifest：仓库本地 TOML，默认 `./.harness/harness.toml`，声明版本、配置的源、显式目标、`[[dir]]` 和扩展。
- 源目录：配置的 `[[resources]]` 源下的持久资源，以及配置的 `[[dir]]` 源下的仓库相对输出。
...
- 投影边界：`.harnessIgnore`，包括根、源本地、profile 本地、目标输出本地和 `.harnessMutable` 规则。
