...
socialTitle: 为什么 .harness 要分离源目录和 runtime 文件夹
description: 为什么标准要把持久源目录与实时 runtime 表面分开。
socialDescription: 将 runtime 文件夹视为生成投影，而不是规范源的设计理由。
canonicalPath: /specifications/v1/rationale/
slug: rationale
order: 1
locale: zh-cn
sectionCode: "01"
summary: 为什么标准要把持久源目录与实时 runtime 表面分开。
llmSummary: 解释为什么实时智能体 runtime 文件夹应当是派生输出，而 .harness 保持为可审查事实来源。
...
智能体 runtime 需要实时文件夹。团队需要稳定、可审查的源布局，用来保存这些 runtime 消费的资源。Harness config 分离这两个责任，而不规定产品应用模型。

源目录位于配置过的源根目录下，默认约定为 `./.harness`。`./.agents`、`./.claude` 和 `./.cursor` 等 runtime 表面仍然是 runtime 读取的实时文件夹。激活把审查过的目录视图投影到这些表面。

## 解决的问题

- 多个 runtime 文件夹里的近似副本容易漂移。
- runtime 写出的文件很难作为持久源来审查。
...
- 声明目标：例如 `./.agents` 或 `./.claude` 的 runtime 文件夹，只有出现在 manifest 中才接收投影。
