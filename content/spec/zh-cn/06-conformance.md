...
- 仓库一致性：仓库在所选仓库本地 manifest 中声明 `version = 1`、保持 source 路径为仓库本地，并在配置过的 resources 源下存储持久 target 资源。Resources 和 dir 源路径可以使用仓库本地路径模式并扩展到已存在的 source 目录。声明的 target 可以使用显式外部 `parent`，包括路径模式，因为 target 是输出，不是 source root。
...
- Target 一致性：`[[targets]]` 条目包含必需的静态 target-local `path`，并且可以包含显式 `parent`。匹配的 override 文件夹从 `path` 的第一段推断；`parent` 只选择物理输出 parent，并且可以扩展为多个具体 parent。没有 target 可以指向 `.harness`、与配置过的源根重叠、与另一个解析后的 target root 重叠或重新声明资源映射。为未来 v1 修订保留的未知键被容忍并作为信息诊断显示。
...
- 投影一致性：激活应用 `.harnessIgnore` 排除和 `.harnessMutable` 仅初始化所有权规则，包括适用时的源本地、profile 本地和目标输出本地 ignore 文件，区分忽略文件与 runtime 所有的 mutable 文件，把每个声明的 target 视为拷贝投影，在产生源仍存在时区分孤立的受管理输出和未管理 target 条目，并对相同的规范输入产生相同的受管理投影子集。
- 工具一致性：实现在写入之前报告激活计划，列出创建、更新、请求的删除、保留的文件、保留的未管理项、孤立的受管理输出和 mutable 跳过的文件，并从不把活动 target 文件夹作为真理源读取。当工具提供路径自省时，该解释是只读的，并从与激活相同的规范输入派生。
...
- `[[targets]]` 条目包含必需的静态 target-local 路径和仅用于输出放置的可选 parent。
...
- Override MUST 从 target 路径派生，而不是从 target parent 派生。
- 所选 manifest MUST 把 target 条目限制为必需的静态 target-local 路径、可选 parent 和为未来 v1 修订保留的未识别键。Target MUST NOT 重新定义资源、模式或 override 名称。顶级 `[[resources]]` 和 `[[dir]]` 表声明有序的仓库本地 source root，并且它们的路径模式 MUST 只扩展到仓库内已存在的真实目录。
- 激活 SHOULD 从投影派生。
...
- `[[targets]].parent` 模式 MAY 扩展为多个具体 target parent，但 `[[targets]].path` MUST 对每个具体 target 保持静态和显式。
