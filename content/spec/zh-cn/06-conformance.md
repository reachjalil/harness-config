...
description: 面向仓库、资源、目标、profile、投影、路径解释和工具的可测试声明。
...
locale: zh-cn
sectionCode: "06"
summary: 面向仓库、资源、目标、profile、投影、路径解释和工具的可测试声明。
llmSummary: 列出仓库形状、资源路径、目标投影、覆盖、ignore、profile、扩展、激活输出和只读路径解释的一致性期望。
...
- 投影一致性：激活应用根、源本地、目标输出本地和 `.harnessMutable` 的 `.harnessIgnore`。
- 工具一致性：工具在写入前报告计划，并且从不把 harness surface 当作事实来源。如果工具提供路径解释，该解释必须是只读的，并使用与激活相同的选中 manifest、配置源、profile、ignore 规则、mutable 策略和投影模型。
