...
Los objetivos declarados son superficies de harness vivas, no repositorios fuente. Un repositorio MAY hacer commit de las salidas objetivo generadas, ignorarlas en Git o mezclar archivos gestionados con commit con controles locales, mientras la fuente de verdad revisada permanezca en el manifiesto seleccionado y las raíces fuente configuradas. Esto permite a los equipos experimentar en `.agents`, `.claude`, `.cursor` u otra superficie sin promover ediciones runtime de vuelta al layout fuente canónico.

Cuando un repositorio ignora en Git las salidas objetivo generadas, SHOULD mantener instrucciones de activación con seguimiento, como una nota de instrucciones raíz, paso README o script, para que un nuevo checkout pueda validar y regenerar esas salidas. Las raíces fuente configuradas compartidas que hacen los objetivos reproducibles SHOULD permanecer con seguimiento; las raíces locales privadas o experimentales como `.harness/local/` MAY ignorarse en Git cuando el repositorio las trata intencionalmente como superposiciones locales al desarrollador.
...
3. selectores `.harnessProfile` y superposiciones `.harnessProfileRoot` activas, incluidas las declaraciones `.harnessProfileIsolation` locales al perfil,
...
La clasificación no gestionada se basa en el grafo fuente configurado actual, no en el historial de activación previo. Una entrada objetivo cuyo archivo fuente anterior fue eliminado, cuya raíz fuente ya no está configurada, o cuyo camino fuente ahora está excluido por `.harnessIgnore` no tiene productor fuente actual en el contrato base de proyección v1 y por lo tanto es no gestionada. Lo mismo aplica a archivos locales escritos directamente en un objetivo declarado.
...
La clasificación huérfana requiere un productor no activo actual. Una fuente normal de recursos o dir que está activa para un camino de salida forma parte de la proyección activa; si se elimina del grafo fuente configurado, ya no prueba propiedad gestionada de ninguna entrada objetivo obsoleta. Las raíces de perfil y las fuentes de override específicas de objetivo pueden permanecer configuradas mientras no están seleccionadas para un camino de salida particular, así que pueden probar una salida gestionada huérfana sin requerir historial de activación.
...
- Si el directorio marcador está anidado más profundo dentro de una fuente de recursos configurada o una fuente dir configurada, ese directorio marcador superpone su directorio padre. Esto permite a los elementos de recursos llevar perfiles locales portables. Por ejemplo, bajo un camino de recursos convencional, `.harness/resources/skills/example/focusedProfile/.harnessProfileRoot` superpone `.harness/resources/skills/example`, por lo que `.harness/resources/skills/example/focusedProfile/SKILL.md` reemplaza el lógico `.harness/resources/skills/example/SKILL.md` cuando ese perfil está activo.
- De lo contrario, un directorio marcador bajo `./.harness` superpone `./.harness`. Esto soporta layouts de pack como `.harness/packs/deploy/.harnessProfileRoot` con hijos como `resources/` y `dir/`.

Las raíces de perfil MAY incluir un archivo opcional `.harnessProfileIsolation`. El archivo es TOML UTF-8 y MUST usar `version = 1`. MAY declarar patrones estilo gitignore bajo `[isolate]`:
...
Los archivos `.harnessIgnore` fuente-locales que son ancestros físicos de una raíz de perfil también se aplican antes de que la raíz de perfil sea mapeada a su camino de superposición lógica. Por ejemplo, `.harness/packs/.harnessIgnore` puede excluir los metadatos `.harness/packs/deploy/**/.harness-cache/` del perfil `deploy` activo incluso cuando los archivos bajo esa raíz de perfil superponen caminos lógicos como `.harness/resources` o `.harness/dir`.
...
- Los equipos MAY ignorar en Git las superficies de harness vivas porque son salidas generadas; hacerlo no cambia la fuente de verdad ni el contrato de declaración de objetivo.
- Los repositorios que ignoran en Git las superficies de harness vivas SHOULD mantener instrucciones de activación con seguimiento y SHOULD NOT ignorar en Git las raíces fuente configuradas compartidas requeridas para regenerar esas superficies. Las raíces fuente locales al desarrollador MAY ser ignoradas por Git cuando están intencionalmente fuera de la fuente de verdad compartida.
