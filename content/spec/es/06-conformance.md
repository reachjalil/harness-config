...
- Conformidad de objetivo: una entrada `[[targets]]` contiene solo un camino local al repositorio. La carpeta de override coincidente se infiere del primer segmento de camino. Ningún objetivo puede apuntar a `.harness`, superponerse con una raíz fuente configurada o re-declarar mapeos de recursos.
...
- Las entradas `[[targets]]` contienen solo caminos locales al repositorio.
...
- El manifiesto seleccionado MUST mantener las entradas de objetivo solo-camino. Los objetivos MUST NOT redefinir recursos, modos o nombres de override. Las tablas `[[resources]]` y `[[dir]]` de nivel superior declaran raíces fuente ordenadas.
- La activación SHOULD derivarse de la proyección.
- La activación MUST ser idempotente para los mismos árboles fuente configurados, manifiesto, overrides, reglas `.harnessIgnore`, reglas `.harnessMutable`, elección de limpieza, política de mutables y política de enlace simbólico objetivo.
...
Las herramientas conformes SHOULD reportar códigos de diagnóstico legibles por máquina junto con los mensajes humanos. El catálogo de códigos v1 se mantiene en [`./DIAGNOSTICS.md`](./DIAGNOSTICS.md). Las herramientas que emiten un código `harness.*` MUST usar un código de ese catálogo. Las herramientas MAY emitir códigos en su propio espacio de nombres (por ejemplo `my-tool.*`) para condiciones fuera del estándar.
