...
- Conformidad de repositorio: un repositorio declara `version = 1` en el manifiesto local seleccionado, mantiene los caminos fuente locales al repositorio y almacena recursos objetivo persistentes bajo fuentes de recursos configuradas. Los caminos de fuentes de recursos y dir pueden usar patrones de ruta locales al repositorio que se expanden a directorios fuente existentes. Un objetivo declarado puede usar un `parent` externo explícito, incluido un patrón de ruta, porque los objetivos son salidas, no raíces fuente.
...
- Conformidad de objetivo: una entrada `[[targets]]` contiene un `path` target-local estático requerido y puede contener un `parent` explícito. La carpeta de override coincidente se infiere del primer segmento de `path`; `parent` solo elige el padre físico de salida y puede expandirse a varios padres concretos. Ningún objetivo puede apuntar a `.harness`, superponerse con una raíz fuente configurada, superponerse con otra raíz objetivo resuelta o re-declarar mapeos de recursos. Las claves desconocidas reservadas para futuras revisiones v1 se toleran y se muestran como diagnósticos informacionales.
...
- Las entradas `[[targets]]` contienen caminos target-local estáticos requeridos y padres opcionales solo para ubicación de salida.
...
- Los overrides MUST derivarse del camino objetivo, no del padre objetivo.
- El manifiesto seleccionado MUST mantener las entradas de objetivo limitadas a caminos target-local estáticos requeridos, padres opcionales y claves no reconocidas reservadas para futuras revisiones v1. Los objetivos MUST NOT redefinir recursos, modos o nombres de override. Las tablas `[[resources]]` y `[[dir]]` de nivel superior declaran raíces fuente locales al repositorio ordenadas, y sus patrones de ruta MUST expandirse solo a directorios reales existentes dentro del repositorio.
...
- Las implementaciones MUST reportar las salidas gestionadas huérfanas como distintas de las entradas objetivo no gestionadas cuando la fuente productora no activa todavía existe. Cambiar el perfil activo y volver a activar MUST reportar las salidas ahora no seleccionadas del perfil anterior como salidas gestionadas huérfanas, preservarlas por defecto, eliminar las no editadas solo bajo limpieza explícita y nunca eliminar una huérfana editada localmente ni una entrada genuinamente no gestionada.
...
- Las implementaciones MUST soportar `.harnessProfileIsolation` cuando exista dentro de una raíz de perfil activa, suprimiendo candidatos de recursos y dir no-perfil coincidentes mientras preservan raíces de perfil activas con el mismo nombre y rutas no relacionadas. Los patrones de aislamiento MUST usar el mismo modelo de evaluación ordenada con negación y última-coincidencia-gana que las reglas de camino `.harnessIgnore`.
...
- Las carpetas objetivo declaradas MUST NOT apuntar a `./.harness`, superponerse con raíces fuente configuradas o superponerse con otra raíz objetivo resuelta.
- Los patrones `[[targets]].parent` MAY expandirse a varios padres objetivo concretos, pero `[[targets]].path` MUST permanecer estático y explícito para cada objetivo concreto.
