...
- Conformidad de recurso: un recurso vive bajo la fuente configurada y las sobrescrituras son carpetas dentro del recurso.
- Conformidad de destino: cada `[[targets]]` contiene solo una ruta repo-local y no apunta hacia `.harness`.
- Conformidad `[dir]`: `[dir]` compone hojas `.harnessComposable` y copia otros archivos.
